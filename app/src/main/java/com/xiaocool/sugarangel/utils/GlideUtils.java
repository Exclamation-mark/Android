...
public class GlideUtils {
    ...
    public static void loadImageView(Context mContext, String path, ImageView mImageView) {
        Glide.with(mContext).load(path).error(R.mipmap.ic_launcher).into(mImageView);
    }
    //加载圆角
    public static void loadRectangImageView(Context mContext, String path, ImageView mImageView) {
//        Glide.with(mContext).load(path).error(R.mipmap.ic_launcher).into(mImageView);
        Glide.with(mContext).load(path).error(R.mipmap.ic_launcher).transform(new GlideRoundTransform(mContext, 10)).into(mImageView);
    }
    //显示头像
    public static void showAvatar(Context mContext,String path,ImageView mImageView){
        Glide.with(mContext).load(path).error(R.drawable.touxiang).into(mImageView);
    }
    ...
    public static void loadImageView(Context mContext, File file, ImageView mImageView) {
        Glide.with(mContext).load(file).skipMemoryCache(true).into(mImageView);
    }
    //加载指定大小
